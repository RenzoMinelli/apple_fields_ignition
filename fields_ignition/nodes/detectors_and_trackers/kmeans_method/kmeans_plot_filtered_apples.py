...
class KMeansPlotting:
  ...
  def get_depths(self, timestamp, bounding_boxes):
      ...
      for bb_center in bounding_boxes[timestamp]:
          ...
          depths.append([bb_id, depth, bb_center[0], bb_center[1]])
  ...
  def filter_depths(self, depths, threshold):
      green_depths = []
      red_depths = []
      for depth in depths:
  ...
  def track_filter_and_count(self, working_directory):
...
def plot():
    ...
    plotting = KMeansPlotting()
    plotting.track_filter_and_count(args.working_directory)
